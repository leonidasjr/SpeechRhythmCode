...
for y from 1 to numberOfFiles
	...
	if voice_quality_parameters = 1
		numberOfVowels = Get number of intervals... 'v_C_#_tier'

		for i from 2 to numberOfVowels - 1
			select TextGrid 'soundFile$'
			label_vowel$ = Get label of interval... 'v_C_#_tier' 'i'
 			if label_vowel$ = "V"
 				vowel$ = label_vowel$
 				start_time = Get start point... 'v_C_#_tier' 'i'
        		end_time = Get end point... 'v_C_#_tier' 'i'
	...
	chunk$ = Get label of interval... 'chunk_tier' 'x'
		...
		select TextGrid 'soundFile$'
			nVC = Get interval at time... 'v_C_#_tier' 'endChunk'
			start = Get interval at time... 'v_C_#_tier' 'startChunk'
			tiniant = 0
			sSil = 0
			tzero = Get start point... 'v_C_#_tier' 2
			cpt = 0
		for i from start to nVC
  			label$ = Get label of interval... 'v_C_#_tier' 'i'
  			if label$ = "_" or label$ = "#" or label$ = "PAUSE" or label$ = "sp" or label$ = "<sp>"
   				cpt = cpt + 1
   				tini = Get start point... 'v_C_#_tier' 'i'
   				tfin = Get end point... 'v_C_#_tier' 'i'
	...
	#nVC = Get number of intervals... 'v_C_#_tier'
	cptV = 0
	cptC = 0
	sVdur = 0
	sCdur = 0
	...
	procedure VC_duration
    	for i from start to nVC - 1
 			label$ = Get label of interval... 'v_C_#_tier' 'i'
  			if label$ == "V"
    			cptV = cptV + 1
  	 			itime = Get starting point... 'v_C_#_tier' 'i'
  	 			ftime = Get end point... 'v_C_#_tier' 'i'
	...
	for i from start to nVC - 2
 	label$ = Get label of interval... 'v_C_#_tier' 'i'
 	  if label$ == "V"
  	   ...
  	   nextI = i + 1
  	   nextItime = Get starting point... 'v_C_#_tier' 'nextI'
  	   nextFtime = Get end point... 'v_C_#_tier' 'nextI'
...
startVC = Get interval at time... 'v_C_#_tier' 'startChunk'
nVC = Get number of intervals... 'v_C_#_tier'
for i from startVC to nVC - 1
label$ = Get label of interval... 'v_C_#_tier' 'i'
  if label$ == "#" or label$ == "PAUSE" or label$ == "_" or label$ == "sp" or label$ == "<sp>" or label$ == "sil" or label$ == ""
   cptPAUSE = cptPAUSE + 1
   start_pause = Get starting point... 'v_C_#_tier' 'i'
   end_pause = Get end point... 'v_C_#_tier' 'i'
