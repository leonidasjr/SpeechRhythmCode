...
# Copyright (C) 2019, 2022, 2023 SILVA JR., Leonidas. & BARBOSA, Plinio Almeida. version 1.07
# SpeechRhythmExtractor (version 1.08)
...
#
# Inclusion of Macro Rhythm measures by Sun Ah-Jun (2014)
#
...
form set the input parameters
	...
	natural VVtier 1
 	## integer PhoneTier (in update...)
	natural VCtier 2
	## natural SentenceTier (in update...)
	natural WordTier 4
	natural ChunkTier 5
	## natural ToneTier (in update...)
endform
	...
	chunk$ = Get label of interval... 'chunkTier' 'x'
		if chunk$ <> ""
			startChunk = Get starting point... 'chunkTier' 'x'
 			...
 			select TextGrid 'soundFile$'
 			word_start = Get interval at time... 'wordTier' 'startChunk'
			word_end = Get interval at time... 'wordTier' 'endChunk'
			nWords = Get number of intervals... 'wordTier'

			word_counter = 0
			for w from word_start to word_end
				w_label$ = Get label of interval... 'wordTier' 'w'
				if w_label$ <> ""
					startWord = Get starting point... 'wordTier' 'w'
 					endWord = Get end point... 'wordTier' 'w'
			...
			# The original metric Frequency Index (MacR_Freq - JUN, 2014) takes the number of f0 peaks
